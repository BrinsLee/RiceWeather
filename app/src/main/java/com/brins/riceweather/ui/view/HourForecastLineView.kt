...
class HourForecastLineView @JvmOverloads constructor(
    context: Context,
    ...
    var forecastLists: List<HourForecast>? = null
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0..5) {
            canvas.drawText(
                forecastLists!![i + 1].temperature,
                (i * (xOffset) + xItemX), xItemY - yInterval * 2f, axisPaint
            )

            yPointMax[i] = (xItemY - forecastLists!![i + 1].tempInt() * 2 - yInterval / 2)
